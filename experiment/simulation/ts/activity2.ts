...
let row_count = 1;
let scale_img: Chemistry.Custome_image;
let assembly_img: Chemistry.Custome_image;
let line_out_1: Chemistry.Line;
let line_out_2: Chemistry.Line;
let curve_out: Chemistry.Circle;
...
let va_inp: HTMLInputElement;
let I_inp: HTMLInputElement;
let beta_inp: HTMLInputElement;
let r_inp: HTMLInputElement;
...
function activity2() {
   ...
   <div id="B-div" style="position:absolute;left:4vw; top:10vw; font-size:25px; font-weight:bold">
	...
	scale_img = new Chemistry.Custome_image(
		scale,
		new Chemistry.Point(750, 295),
		1152 * 0.98,
		230 * 0.98,
		canvas1
	);
	...
	assembly_img = new Chemistry.Custome_image(
		assembly,
		new Chemistry.Point(630, 400),
		5766 * 0.21,
		3233 * 0.21,
		canvas1
	);
	...
	if (isNaN(r) || r === Infinity || r === 0) {
		line_out_2 = new Chemistry.Line(
			new Chemistry.Point(122.5, 0),
			new Chemistry.Point(122.5, 736),
			1,
			'black',
			canvas2
		);
		...
		curve_out = new Chemistry.Circle(
			new Chemistry.Point(122.5 + r_canvas2, 0),
			r_canvas2,
			canvas2
		);
...
function add_description_btn() {
	...
	pp.addtorightpannel(
		`<button id="panel1_btn" class="btn btn-primary" onclick="a2_verify_reading()" style="position: absolute; bottom: 12vh; width: 91%;">Verify</button>`,
		3
	);
}

function calculate_beta(ele: HTMLInputElement): void {
	let b: HTMLDivElement = <HTMLDivElement>document.getElementById('B-div');
	let label: HTMLLabelElement = <HTMLLabelElement>(
		document.getElementById('current-label')
	);
...
function calculate_r_2(ele?: HTMLInputElement): void {
	let label: HTMLLabelElement = <HTMLLabelElement>(
		document.getElementById('voltage-label')
	);

	va_inp = <HTMLInputElement>(
		document.getElementById(`a2-va-inp-${row_count}`)
	);
	I_inp = <HTMLInputElement>document.getElementById(`a2-I-inp-${row_count}`);
	beta_inp = <HTMLInputElement>(
		document.getElementById(`a2-B-inp-${row_count}`)
	);
	r_inp = <HTMLInputElement>document.getElementById(`a2-r-inp-${row_count}`);
...
function a2_verify_reading() {
	...
	if (row_count === 5) {
		let vol: HTMLInputElement = <HTMLInputElement>(
			document.getElementById('voltage')
		);
		let cur: HTMLInputElement = <HTMLInputElement>(
...
function a2_check_duplicate_reading(): boolean {
	for (let i = 0; i < data_table_1.length; i++) {
		if (v_a === data_table_1[i][0] && I === data_table_1[i][1]) return true;
	}

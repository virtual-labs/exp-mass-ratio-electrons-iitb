function activity4() {
	...
	let text = `
      <div style="margin-top:10px;">
         <h4>Standard value of e by m = ${
				standard_e_by_m / 10 ** 11
			}&times; 10<sup>11</sup> C/kg </h4>
      </div>
      <br>
      <br>
...
function verify_cal_em() {
	let val: HTMLInputElement = <HTMLInputElement>(
		document.getElementById('cal-em')
	);

	let div: HTMLDivElement = <HTMLDivElement>(
		document.getElementById('cal-em-div')
	);

	console.log(e_by_m_mean / 10 ** 11);

	if (!verify_values(parseFloat(val.value), e_by_m_mean / 10 ** 11)) {
		alert('Calculated value and entered value are different');
		return;
	} else {
		div.innerHTML = '';
		div.innerHTML = `
      <h4>Calculated value of e by m = ${parseFloat(
			(e_by_m_mean / 10 ** 11).toFixed(6)
		)} &times; 10<sup>11</sup> C/kg </h4>
      `;
	}
...
function experiment_complete() {
	let btn = <HTMLButtonElement>document.getElementById('cal-em-btn');

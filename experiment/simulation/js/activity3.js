function activity3() {
    ...
    pp.showdescription(`
      ...
      `, 3);
    pp.addtorightpannel(`<button  class="btn btn-primary" onclick="a3_verify_table()" style="position: absolute; bottom: 12vh; width: 91%;">Verify</button>`, 3);
    show_panel(3);
    a3_load_table1();
...
function cal_percentage_error() {
    let error = Math.round(Math.abs((e_by_m_mean - standard_e_by_m) / standard_e_by_m) * 100);
...
function a3_load_table1() {
    ...
    for (let i = 0; i < data_table_1.length; i++) {
        tbody += `<tr>
                  ...
                  <td style="width:15vw">${parseFloat(data_table_1[i][2].toFixed(6))}</td>
                  <td style="width:15vw">${parseFloat(data_table_1[i][4].toFixed(1))}</td>
...
function a3_resizeTable() {
    let tab = (document.getElementById('a3-table-1'));
...
function a3_verify_table() {
    let e_by_m_inp;
    for (let i = 0; i < data_table_1.length; i++) {
        e_by_m_inp = (document.getElementById(`a3-e-by-m-inp-${i}`));
        console.log(data_table_1[i][6] / Math.pow(10, 11));
        if (!verify_values(parseFloat(e_by_m_inp.value), data_table_1[i][6] / Math.pow(10, 11))) {
...
function a3_load_data_table() {
    ...
    pp.showdescription(`<p class='discription_text'>Calculations correct. Calculate average e/m.</p>`, 3);
    show_panel(3);
    let tbody = ``;
    for (let i = 0; i < data_table_1.length; i++) {
        tbody += `<tr>
                  ...
                  <td style="width:15vw">${parseFloat(data_table_1[i][4].toFixed(1))}</td>
                  <td style="width:15vw">${data_table_1[i][5].toFixed(6)}</td>
                  <td style="width:15vw">${data_table_1[i][6].toExponential(6)}</td>
            </tr>`;
    }
...
function verify_mean_e_by_m() {
    let mean_em_inp = document.getElementById('mean-em');
    let btn = document.getElementById('mean-em-btn');
    console.log(e_by_m_mean / Math.pow(10, 11));
    if (!verify_values(parseFloat(mean_em_inp.value), e_by_m_mean / Math.pow(10, 11))) {
